...
describe("Task Manager", () => {
  ...
  it("search something", () => {
    const input = "Дизайн";
    cy.get(".form-search").type(input).get("tr").should("have.length", 4);
  });
  it("search nothing", () => {
    cy.get(".form-search").type("{enter}").get("tr").should("have.length", 9);
  });
});
